...
def score_faithfulness(
    ...
    """
    if not chunks_used:
        return {"score": 1, "notes": "No chunks used"}
...
Rate the FAITHFULNESS of the model's answer based ONLY on the provided context chunks.
...
Scale 1 to 5:
5: All information in the answer is supported by the chunks. No hallucinations.
1: The answer contains information not found in the chunks or contradicts them.

Output strictly valid JSON with keys "score" and "reason".
JSON:"""
    ...
    try:
        ...
        if match:
            ...
            return {"score": data.get("score", 3), "notes": data.get("reason", "")}
    except Exception:
        pass
    
    return {"score": 3, "notes": "Evaluation error"}
...
def score_answer_relevance(
    ...
    Dùng LLM-as-Judge.
    """
    if "NOT_IMPLEMENTED" in answer:
        return {"score": 1, "notes": "Answer not implemented"}

    from rag_answer import call_llm
    
    prompt = f"""You are an expert evaluator.
Rate the RELEVANCE of the model's answer to the user's question.
...
def score_completeness(
    ...
    Dùng LLM-as-Judge so sánh với reference answer.
    """
    if "NOT_IMPLEMENTED" in answer or not expected_answer:
        return {"score": 1, "notes": "Missing answer or reference"}

    from rag_answer import call_llm
    
    prompt = f"""Compare the model's answer against the expected gold-standard answer.
Question: {query}
Expected Answer: {expected_answer}
Model Answer: {answer}

Rate COMPLETENESS 1 to 5. Did the model cover all key points of the expected answer?
Output strictly valid JSON with keys "score" and "reason".
JSON:"""
...
if __name__ == "__main__":
    ...
    # --- Grading Run Generator (Script for 17:00) ---
    ...
    if GRADING_FILE.exists():
        ...
        for q in grading_questions:
            # Dùng cấu hình tốt nhất (Variant)
            res = rag_answer(q["question"], **{k:v for k,v in VARIANT_CONFIG.items() if k != 'label'})
            logs.append({
                "id": q["id"],
                "question": q["question"],
                "answer": res["answer"],
        ...
        with open(LOGS_DIR / "grading_run.json", "w", encoding="utf-8") as f:
            json.dump(logs, f, ensure_ascii=False, indent=2)
        print(f"Grading results saved to logs/grading_run.json")
