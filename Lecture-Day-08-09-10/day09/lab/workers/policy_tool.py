...
def analyze_policy(task: str, chunks: list) -> dict:
    ...
    # Orders placed before 01/02/2026 fall under v3 policy (not in our docs → must abstain)
    policy_name = "refund_policy_v4"
    policy_version_note = ""
    import re as _re
    # Detect dates before 01/02/2026 in the task
    _pre_v4_patterns = ["31/01", "30/01", "29/01", "28/01", "trước 01/02", "trước ngày 01/02"]
    if any(p in task_lower for p in _pre_v4_patterns):
        ...
        exceptions_found.append({
            ...
            "rule": "ĐƠN HÀNG NÀY ĐẶT TRƯỚC 01/02/2026 — áp dụng chính sách hoàn tiền PHIÊN BẢN 3, không phải v4. Tài liệu hiện tại CHỈ có v4. KHÔNG THỂ XÁC NHẬN theo v3 — phải báo cáo 'thiếu thông tin v3' thay vì áp dụng v4.",
            "source": "policy_refund_v4.txt",
        })
...
def run(state: dict) -> dict:
    ...
    try:
        ...
        if not chunks and needs_tool:
            mcp_result = _call_mcp_tool("search_kb", {"query": task, "top_k": 8})
        ...
        # Step 4: Nếu task đề cập đến access level cụ thể, gọi check_access_permission
        import re as _re2
        level_match = _re2.search(r"level\s*(\d)", task.lower())
        if needs_tool and level_match:
            access_level = int(level_match.group(1))
            if 1 <= access_level <= 3:
                is_emergency = any(kw in task.lower() for kw in [
                    "emergency", "khẩn cấp", "tạm thời", "2am", "sự cố", "p1"
                ])
                ...
                state["history"].append(
                    f"[{WORKER_NAME}] called MCP check_access_permission(level={access_level}, emergency={is_emergency})"
                )
                # Inject access control info as a retrieved chunk for synthesis
                if mcp_result.get("output") and not mcp_result["output"].get("error"):
                    out = mcp_result["output"]
                    access_text = (
                        f"Level {access_level} Access Control:\n"
                        f"- Phê duyệt bởi: {', '.join(out.get('required_approvers', []))}\n"
                        f"- Số người phê duyệt: {out.get('approver_count', '?')}\n"
                        f"- Emergency bypass: {'CÓ' if out.get('emergency_override') else 'KHÔNG'}\n"
                    )
                    if out.get("notes"):
                        access_text += f"- Ghi chú: {'; '.join(out['notes'])}\n"
                    access_chunk = {
                        "text": access_text,
                        "source": "access_control_sop.txt",
                        "score": 0.95,
                        "metadata": {"section": "mcp_check_access"},
                    }
                    state["retrieved_chunks"] = [access_chunk] + state.get("retrieved_chunks", [])
                    if "access_control_sop.txt" not in state.get("retrieved_sources", []):
                        state.setdefault("retrieved_sources", []).append("access_control_sop.txt")
