...
def preprocess_document(raw_text: str, filepath: str) -> Dict[str, Any]:
    ...
    for line in lines:
        if not header_done:
            ...
            if ":" in line:
                # Phải check if key is in the key_map to avoid false positives
                parts = line.split(":", 1)
                key = parts[0].strip()
                val = parts[1].strip()
                ...
                if key in key_map:
                    metadata[key_map[key]] = val
                    continue # Dòng metadata -> skip content_lines
            
            # Gặp section heading đầu tiên hoặc sau các key mà gặp text trống rồi đến text thường
            if line.startswith("==="):
                header_done = True
                content_lines.append(line)
            elif len(content_lines) > 5: # Safety break if too many lines without keys
                header_done = True
                content_lines.append(line)
            else:
                # Vẫn đang ở vùng có thể là title hoặc header lines
                content_lines.append(line)
        else:
            content_lines.append(line)
...
def _split_by_size(
    ...
    for p in paragraphs:
        if len(current_chunk_text) + len(p) + 2 <= chunk_chars:
            ...
            # Start new chunk with overlap from the end of previous chunk if possible
            overlap_prefix = current_chunk_text[-overlap_chars:] if len(current_chunk_text) > overlap_chars else ""
            current_chunk_text = overlap_prefix + p + "\n\n"
...
def get_embedding(text: str) -> List[float]:
    """
    Tạo embedding vector cho một đoạn text dùng Sentence Transformers (local).
    """
